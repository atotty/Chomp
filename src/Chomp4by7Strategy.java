/**
 * Source: https://github.com/mattzh72/APCS-Workspace/blob/master/JMCh12_11Chomp/Chomp4by7Strategy.java 
 * 
 * Some code has been modified for errata and for the purpose of this program. 
 */
...
public class Chomp4by7Strategy implements Strategy
{
    ...
    public Location pickMove(ChompGame game) {
        return (Math.random() < TEMP) ? findBestMove(game) : findRandomMove(game);
    }
    ...
    public Location findRandomMove( ChompGame game )
    {
        ...
        for ( int r = 0; r < rows; r++ )
        {
            ...
            {
                if ( r + c > 0 && !game.isEmpty( r, c ) ) { // if not (0,0) and not empty
                    if (r+1 == rows || c+1 == cols) { // if at edge of array
                        list.add(new Location(r,c));
                    }

                    else if (game.isEmpty(r + 1, c) || game.isEmpty(r, c + 1)) {// if not at edge of array but 
                        list.add(new Location(r,c));                            // edge of non-empty cells
                    }
